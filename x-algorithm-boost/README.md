...
| File | What it does |
|---|---|
| `SKILL.md` | The agent playbook. Claude loads this automatically when a request matches. |
| `account.mjs` | Audits an **account** — surfaces, credibility, stage, binding constraint |
| `feedclone.mjs` | Reproduces a **feed** on another account from a follow list |
| `score.mjs` | Audits, rewrites and A/B-scores a **post** |
| `README.md` | This file |
...
```bash
node account.mjs --followers 0 --account-age-days 2 --posts-per-day 3 --reply-share 0.5
```

```bash
node account.mjs --followers 12000 --verified --actor brand --posts-per-day 2
```
...
```bash
node score.mjs --text "your draft here" --followers 800
```

A/B two drafts, optimizing for conversions rather than reach:

```bash
node score.mjs --goal convert --text "draft A" --text "draft B"
```
...
```bash
node feedclone.mjs --list follows.csv --per-day 15
```
